...
const user_admin_schemas_1 = require("./user-admin.schemas");
const router = (0, async_handler_1.createAsyncRouter)();
router.get('/users', async (_req, res) => {
    res.json(await (0, user_admin_service_1.listUsers)());
});
router.post('/users', (0, middleware_1.validateBody)(user_admin_schemas_1.createUserSchema), async (req, res) => {
    res.status(201).json(await (0, user_admin_service_1.addUser)(req.body, req.user?.role));
});
router.put('/users/:id', (0, middleware_1.validateBody)(user_admin_schemas_1.updateUserSchema), async (req, res) => {
