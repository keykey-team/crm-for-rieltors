...
router.post('/lead-distribution', async (req, res) => res.status(201).json(await (0, lead_distribution_service_1.addLeadDistributionRule)(req.body ?? {})));
router.put('/lead-distribution', async (req, res) => res.json(await (0, lead_distribution_service_1.changeLeadDistributionRule)(req.body ?? {})));
...
router.post('/leads', async (req, res) => res.status(201).json(await (0, lead_service_1.addLead)(req.body ?? {}, req.user?.id)));
router.get('/leads/:id', async (req, res) => res.json(await (0, lead_service_1.getLead)(req.params.id, req.user?.id, req.user?.role)));
router.put('/leads/:id', async (req, res) => res.json(await (0, lead_service_1.changeLead)(req.params.id, req.body ?? {}, req.user?.id, req.user?.role)));
router.delete('/leads/:id', async (req, res) => res.json(await (0, lead_service_1.removeLead)(req.params.id)));
router.post('/leads/bulk', async (req, res) => res.json(await (0, lead_service_1.bulkLeadAction)(req.body ?? {}, req.user?.id, req.user?.role)));
router.post('/leads/import', async (req, res) => res.json(await (0, lead_import_service_1.importLeads)(req.body ?? {}, req.user?.id)));
