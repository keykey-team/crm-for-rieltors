...
const isAdmin = (role) => role === "admin" || role === "director";
router.get("/users", async (_req, res) => {
    res.json(await prisma_1.prisma.user.findMany({
        select: {
            id: true,
            name: true,
            email: true,
...
router.post("/users", async (req, res) => {
    ...
    const user = await prisma_1.prisma.user.create({
        data: {
            name: b.name,
            email: b.email,
            password: await bcryptjs_1.default.hash(String(b.password ?? ""), 12),
            role: b.role ?? "agent",
            phone: b.phone ?? null,
...
router.put("/users/:id", async (req, res) => {
    if (!isAdmin(req.user?.role))
        return res.status(403).json({ error: "Forbidden" });
    const b = req.body ?? {};
    res.json(await prisma_1.prisma.user.update({
...
router.get("/settings/profile", async (req, res) => {
    res.json(await prisma_1.prisma.user.findUnique({
...
router.put("/settings/profile", async (req, res) => {
    ...
    if (b.newPassword && String(b.newPassword).length >= 6)
        data.password = await bcryptjs_1.default.hash(String(b.newPassword), 12);
    res.json(await prisma_1.prisma.user.update({
        where: { id: req.user.id },
        data,
        select: {
...
router.get("/funnel-stages", async (_req, res) => res.json(await prisma_1.prisma.funnelStage.findMany({
    where: { isActive: true },
    orderBy: { order: "asc" },
})));
router.post("/funnel-stages", async (req, res) => {
    const maxOrder = await prisma_1.prisma.funnelStage.aggregate({
        _max: { order: true },
    });
    res
        .status(201)
        .json(await prisma_1.prisma.funnelStage.create({
...
router.put("/funnel-stages", async (req, res) => {
    ...
    if (Array.isArray(b.stages)) {
        await prisma_1.prisma.$transaction(b.stages.map((s) => prisma_1.prisma.funnelStage.update({
            where: { id: s.id },
            data: { order: s.order },
        })));
    ...
    const { id, ...data } = b;
    res.json(await prisma_1.prisma.funnelStage.update({ where: { id }, data }));
});
router.delete("/funnel-stages", async (req, res) => {
    const id = String(req.query.id ?? "");
    if (!id)
        return res.status(400).json({ error: "id required" });
    const s = await prisma_1.prisma.funnelStage.findUnique({ where: { id } });
    if (!s)
    ...
    await prisma_1.prisma.funnelStage.update({ where: { id }, data: { isActive: false } });
    res.json({ ok: true });
});
router.get("/deal-custom-fields", async (_req, res) => res.json(await prisma_1.prisma.dealCustomField.findMany({
    where: { isActive: true },
    orderBy: { order: "asc" },
})));
router.post("/deal-custom-fields", async (req, res) => res
    .status(201)
    .json(await prisma_1.prisma.dealCustomField.create({ data: req.body ?? {} })));
router.put("/deal-custom-fields", async (req, res) => {
    const b = req.body ?? {};
    if (Array.isArray(b.items)) {
        await prisma_1.prisma.$transaction(b.items.map((i) => prisma_1.prisma.dealCustomField.update({
            where: { id: i.id },
            data: { order: i.order },
        })));
        return res.json({ ok: true });
    ...
    const { id, ...data } = b;
    res.json(await prisma_1.prisma.dealCustomField.update({ where: { id }, data }));
});
router.delete("/deal-custom-fields", async (req, res) => {
    const id = String(req.query.id ?? "");
    if (!id)
        return res.status(400).json({ error: "id required" });
    await prisma_1.prisma.dealCustomField.update({
        where: { id },
        data: { isActive: false },
    });
    res.json({ ok: true });
...
router.get("/dictionaries", async (req, res) => {
    ...
    res.json(await prisma_1.prisma.dictionary.findMany({
        where,
        orderBy: [{ category: "asc" }, { order: "asc" }],
    }));
});
router.post("/dictionaries", async (req, res) => res
    .status(201)
    .json(await prisma_1.prisma.dictionary.create({ data: req.body ?? {} })));
router.put("/dictionaries", async (req, res) => {
    const b = req.body ?? {};
    if (Array.isArray(b.items)) {
        await prisma_1.prisma.$transaction(b.items.map((i) => prisma_1.prisma.dictionary.update({
            where: { id: i.id },
            data: { order: i.order },
        })));
        return res.json({ ok: true });
    ...
    const { id, ...data } = b;
    res.json(await prisma_1.prisma.dictionary.update({ where: { id }, data }));
});
router.delete("/dictionaries", async (req, res) => {
    const id = String(req.query.id ?? "");
    if (!id)
        return res.status(400).json({ error: "id required" });
    await prisma_1.prisma.dictionary.update({ where: { id }, data: { isActive: false } });
    res.json({ ok: true });
});
router.get("/activity-log", async (req, res) => {
    ...
    res.json(await prisma_1.prisma.activityLog.findMany({
        where,
        orderBy: { createdAt: "desc" },
        take: 50,
...
router.get("/templates", async (req, res) => {
    ...
    if (type)
        where.type = type;
    res.json(await prisma_1.prisma.template.findMany({
        where,
        orderBy: { createdAt: "desc" },
...
router.post("/templates", async (req, res) => {
    ...
    res
        .status(201)
        .json(await prisma_1.prisma.template.create({
        data: {
            name: b.name,
            type: b.type ?? "message",
...
router.put("/templates/:id", async (req, res) => {
    const b = req.body ?? {};
    res.json(await prisma_1.prisma.template.update({
        where: { id: req.params.id },
        data: {
            ...(b.name !== undefined ? { name: b.name } : {}),
...
router.delete("/templates/:id", async (req, res) => {
    await prisma_1.prisma.template.delete({ where: { id: req.params.id } });
    res.json({ success: true });
});
router.get("/knowledge-base", async (req, res) => {
    ...
    if (category)
        where.category = category;
    res.json(await prisma_1.prisma.knowledgeArticle.findMany({
        where,
        orderBy: { createdAt: "desc" },
        take: 100,
...
router.post("/knowledge-base", async (req, res) => {
    ...
    res
        .status(201)
        .json(await prisma_1.prisma.knowledgeArticle.create({
        data: {
            title: b.title,
            content: b.content,
            category: b.category ?? "general",
...
router.put("/knowledge-base/:id", async (req, res) => {
    const b = req.body ?? {};
    res.json(await prisma_1.prisma.knowledgeArticle.update({
        where: { id: req.params.id },
        data: {
...
router.delete("/knowledge-base/:id", async (req, res) => {
    await prisma_1.prisma.knowledgeArticle.delete({ where: { id: req.params.id } });
    res.json({ success: true });
});
router.get("/dashboard/stats", async (req, res) => {
    const ownership = req.user?.role === "admin" || req.user?.role === "director"
        ? {}
    ...
    const [totalLeads, newLeads, totalDeals, activeDeals, totalProperties, totalTasks, pendingTasks, todayTasks,] = await Promise.all([
        prisma_1.prisma.lead.count({ where: ownership }),
        prisma_1.prisma.lead.count({ where: { ...ownership, status: "new" } }),
        prisma_1.prisma.deal.count({ where: ownership }),
        ...
        prisma_1.prisma.task.count({ where: ownership }),
        prisma_1.prisma.task.count({ where: { ...ownership, status: "pending" } }),
    ...
    const recentLeads = await prisma_1.prisma.lead.findMany({
        where: ownership,
        take: 5,
        orderBy: { createdAt: "desc" },
        select: {
    ...
    const dealsByStage = await prisma_1.prisma.deal.groupBy({
        by: ["stage"],
        _count: { id: true },
        where: ownership,
    });
...
router.get("/analytics/extended", async (req, res) => {
    ...
    const avgResponseMs = leads.length
        ? leads.reduce((s, l) => s + (Date.now() - new Date(l.createdAt).getTime()), 0) / leads.length
        : 0;
...
router.get("/communications", async (req, res) => {
    ...
    res.json(await prisma_1.prisma.communication.findMany({
        where: { leadId },
        include: { user: { select: { id: true, name: true } } },
        orderBy: { createdAt: "desc" },
    }));
});
router.post("/communications", async (req, res) => res
    .status(201)
    .json(await prisma_1.prisma.communication.create({
    data: { ...(req.body ?? {}), userId: req.user?.id ?? null },
    include: { user: { select: { id: true, name: true } } },
})));
router.get("/aftercare-plans", async (_req, res) => res.json(await prisma_1.prisma.aftercarePlan.findMany({
...
router.put("/aftercare-plans", async (req, res) => {
    ...
    await prisma_1.prisma.$transaction(b.items.map((i) => prisma_1.prisma.aftercarePlan.update({
        where: { id: i.id },
        data: { order: i.order },
    })));
    res.json({ ok: true });
...
router.post("/aftercare-plans", async (req, res) => {
    ...
    res
        .status(201)
        .json(await prisma_1.prisma.aftercarePlan.create({
        data: {
            ...data,
...
router.put("/aftercare-plans/:id", async (req, res) => {
    ...
    if (Array.isArray(steps)) {
        await prisma_1.prisma.aftercareStep.deleteMany({ where: { planId: req.params.id } });
        if (steps.length)
            await prisma_1.prisma.aftercareStep.createMany({
                data: steps.map((s) => ({ ...s, planId: req.params.id })),
            });
    }
    res.json(await prisma_1.prisma.aftercarePlan.update({
        where: { id: req.params.id },
        data,
        include: { steps: { orderBy: { order: "asc" } } },
    }));
...
router.get("/property-units", async (req, res) => {
    ...
    res.json(await prisma_1.prisma.propertyUnit.findMany({
        where: { propertyId },
        orderBy: [{ section: "asc" }, { floor: "desc" }, { unitNumber: "asc" }],
    }));
});
router.post("/property-units", async (req, res) => res
    .status(201)
    .json(await prisma_1.prisma.propertyUnit.create({ data: req.body ?? {} })));
router.put("/property-units", async (req, res) => {
    const { id, ...data } = req.body ?? {};
    res.json(await prisma_1.prisma.propertyUnit.update({ where: { id }, data }));
});
router.delete("/property-units", async (req, res) => {
    const id = typeof req.query.id === "string" ? req.query.id : "";
    if (!id)
        return res.status(400).json({ error: "id required" });
    await prisma_1.prisma.propertyUnit.delete({ where: { id } });
    res.json({ ok: true });
});
router.get("/files", async (req, res) => {
    ...
    res.redirect(await (0, s3_1.getFileUrl)(path, path.includes("/public/")));
});
router.post("/upload/presigned", async (req, res) => {
    const { fileName, contentType, isPublic } = req.body ?? {};
    if (!fileName || !contentType)
        return res.status(400).json({ error: "Missing fileName or contentType" });
    res.json(await (0, s3_1.generatePresignedUploadUrl)(String(fileName), String(contentType), Boolean(isPublic ?? true)));
});
router.get("/lead-distribution", async (_req, res) => res.json(await prisma_1.prisma.leadDistributionRule.findMany({
    include: { assignTo: { select: { id: true, name: true } } },
    orderBy: { priority: "desc" },
})));
router.post("/lead-distribution", async (req, res) => res
    .status(201)
    .json(await prisma_1.prisma.leadDistributionRule.create({
    data: req.body ?? {},
    include: { assignTo: { select: { id: true, name: true } } },
})));
router.put("/lead-distribution", async (req, res) => {
    const b = req.body ?? {};
    if (Array.isArray(b.items)) {
        await prisma_1.prisma.$transaction(b.items.map((i) => prisma_1.prisma.leadDistributionRule.update({
            where: { id: i.id },
    ...
    const { id, ...data } = b;
    res.json(await prisma_1.prisma.leadDistributionRule.update({
        where: { id },
        data,
        include: { assignTo: { select: { id: true, name: true } } },
    }));
...
router.delete("/lead-distribution", async (req, res) => {
    ...
    if (!id)
        return res.status(400).json({ error: "id required" });
    await prisma_1.prisma.leadDistributionRule.delete({ where: { id } });
    res.json({ ok: true });
});
router.get("/deals/custom-field-values", async (req, res) => {
    ...
    res.json(await prisma_1.prisma.dealCustomFieldValue.findMany({
        where: { dealId },
        include: { field: true },
    }));
});
router.post("/deals/custom-field-values", async (req, res) => {
    ...
    res.json(await prisma_1.prisma.dealCustomFieldValue.upsert({
        where: { dealId_fieldId: { dealId, fieldId } },
        update: { value },
        create: { dealId, fieldId, value },
    }));
});
router.get("/automations", async (_req, res) => res.json(await prisma_1.prisma.automation.findMany({
    orderBy: { createdAt: "desc" },
    include: { createdBy: { select: { id: true, name: true } } },
})));
router.post("/automations", async (req, res) => {
    ...
    res
        .status(201)
        .json(await prisma_1.prisma.automation.create({
        data: {
            name: b.name,
...
router.put("/automations/:id", async (req, res) => {
    const b = req.body ?? {};
    res.json(await prisma_1.prisma.automation.update({
        where: { id: req.params.id },
        data: {
            ...(b.name !== undefined ? { name: b.name } : {}),
...
router.delete("/automations/:id", async (req, res) => {
    await prisma_1.prisma.automation.delete({ where: { id: req.params.id } });
    res.json({ success: true });
});
router.post("/leads/:id/create-deal", async (req, res) => {
    ...
    const deal = await prisma_1.prisma.deal.create({
        data: {
            title: b.title || `Угода: ${lead.firstName} ${lead.lastName || ""}`.trim(),
            stage: "new_lead",
            leadId: lead.id,
...
router.get("/deals/:id/comments", async (req, res) => res.json(await prisma_1.prisma.dealComment.findMany({
    where: { dealId: req.params.id },
    orderBy: { createdAt: "desc" },
    include: { author: { select: { id: true, name: true, email: true } } },
})));
router.post("/deals/:id/comments", async (req, res) => res
    .status(201)
    .json(await prisma_1.prisma.dealComment.create({
    data: {
...
router.get("/deals/:id/checklist", async (req, res) => res.json(await prisma_1.prisma.dealChecklist.findMany({
    where: { dealId: req.params.id },
    orderBy: { order: "asc" },
})));
router.post("/deals/:id/checklist", async (req, res) => res
    .status(201)
    .json(await prisma_1.prisma.dealChecklist.create({
    data: {
        dealId: req.params.id,
...
router.put("/deals/:id/checklist", async (req, res) => res.json(await prisma_1.prisma.dealChecklist.update({
...
router.get("/events", async (req, res) => {
    ...
    res.json(await prisma_1.prisma.event.findMany({
        where,
...
router.post("/events", async (req, res) => {
    ...
    res
        .status(201)
        .json(await prisma_1.prisma.event.create({
        data: {
            title: b.title,
            description: b.description ?? null,
            type: b.type ?? "meeting",
...
router.put("/events/:id", async (req, res) => {
    const b = req.body ?? {};
    res.json(await prisma_1.prisma.event.update({
        where: { id: req.params.id },
        data: {
            title: b.title,
            description: b.description ?? null,
...
router.get("/leads", async (req, res) => {
    ...
    res.json(await prisma_1.prisma.lead.findMany({
        where,
        orderBy: { createdAt: "desc" },
        take: 200,
        include: {
...
router.post("/leads", async (req, res) => {
    ...
    if (!b.assignedToId) {
        const rules = await prisma_1.prisma.leadDistributionRule.findMany({
            where: { isActive: true },
            orderBy: { priority: "desc" },
        });
        for (const rule of rules) {
            const sourceMatch = !rule.source || rule.source === (b.source || "manual");
            const districtMatch = !rule.district || rule.district === (b.districts || "");
            const typeMatch = !rule.propertyType || rule.propertyType === (b.propertyType || "");
            const needMatch = !rule.needType || rule.needType === (b.needType || "buy");
    ...
    res
        .status(201)
        .json(await prisma_1.prisma.lead.create({
        data: {
...
router.put("/leads/:id", async (req, res) => {
    const b = req.body ?? {};
    const old = await prisma_1.prisma.lead.findUnique({ where: { id: req.params.id } });
    if (!old)
        return res.status(404).json({ error: "Not found" });
    if (!(req.user?.role === "admin" || req.user?.role === "director") &&
        old.assignedToId !== req.user?.id)
        return res.status(403).json({ error: "Forbidden" });
    res.json(await prisma_1.prisma.lead.update({
        where: { id: req.params.id },
        data: {
...
router.post("/leads/bulk", async (req, res) => {
    ...
    if (action === "delete")
        return res.json({
            deleted: (await prisma_1.prisma.lead.deleteMany({ where })).count,
        });
    if (action === "status" && value)
        return res.json({
            updated: (await prisma_1.prisma.lead.updateMany({ where, data: { status: value } })).count,
        });
    if (action === "assign" && value)
        return res.json({
            updated: (await prisma_1.prisma.lead.updateMany({ where, data: { assignedToId: value } })).count,
        });
...
router.post("/leads/import", async (req, res) => {
    ...
    for (let i = 0; i < leads.length; i++) {
        ...
        try {
            ...
            skipped++;
            errors.push(`row ${i + 2}: ${String(e?.message || "error").slice(0, 80)}`);
        }
    }
    res.json({ imported, skipped, errors: errors.slice(0, 10) });
});
router.get("/deals", async (_req, res) => {
    const ownership = _req.user?.role === "admin" || _req.user?.role === "director"
        ? {}
        : { assignedToId: _req.user?.id };
    res.json(await prisma_1.prisma.deal.findMany({
        where: ownership,
        orderBy: { createdAt: "desc" },
        take: 200,
        include: {
...
router.post("/deals", async (req, res) => {
    ...
    res
        .status(201)
        .json(await prisma_1.prisma.deal.create({
        data: {
            title: b.title,
...
router.get("/deals/:id", async (req, res) => {
    const d = await prisma_1.prisma.deal.findUnique({
        where: { id: req.params.id },
        include: {
            lead: true,
            property: true,
...
router.put("/deals/:id", async (req, res) => {
    const b = req.body ?? {};
    res.json(await prisma_1.prisma.deal.update({
        where: { id: req.params.id },
        data: {
            ...(b.title !== undefined ? { title: b.title } : {}),
...
router.get("/properties", async (req, res) => {
    ...
    res.json(await prisma_1.prisma.property.findMany({
        where,
        orderBy: { createdAt: "desc" },
        take: 200,
    }));
...
router.post("/properties", async (req, res) => {
    ...
    res.status(201).json(await prisma_1.prisma.property.create({ data: payload }));
});
router.put("/properties/:id", async (req, res) => {
    const payload = normalizePropertyPayload(req.body);
    res.json(await prisma_1.prisma.property.update({
        where: { id: req.params.id },
        data: payload,
    }));
});
router.delete("/properties/:id", async (req, res) => {
    await prisma_1.prisma.property.delete({ where: { id: req.params.id } });
    res.json({ success: true });
});
router.get("/tasks", async (req, res) => {
    ...
    res.json(await prisma_1.prisma.task.findMany({
        where,
        orderBy: { createdAt: "desc" },
        take: 300,
        include: {
...
router.post("/tasks", async (req, res) => {
    ...
    res.status(201).json(await prisma_1.prisma.task.create({
        data: {
            ...body,
...
router.put("/tasks/:id", async (req, res) => {
    ...
    res.json(await prisma_1.prisma.task.update({
        where: { id: req.params.id },
        data: body,
    }));
});
...
router.put("/settings/brand", async (req, res) => {
    ...
    if (b.brandName !== undefined)
        data.brandName = b.brandName || null;
    if (b.brandLogo !== undefined)
        data.brandLogo = b.brandLogo || null;
    if (b.primaryColor !== undefined)
        data.primaryColor = b.primaryColor || null;
    if (b.themeMode !== undefined)
        data.themeMode = b.themeMode || "light";
    if (b.sidebarGlass !== undefined)
        data.sidebarGlass = !!b.sidebarGlass;
    if (b.sidebarOpacity !== undefined)
        data.sidebarOpacity =
            typeof b.sidebarOpacity === "number" ? b.sidebarOpacity : 1;
    if (b.gradientBg !== undefined)
        data.gradientBg = !!b.gradientBg;
    res.json(await prisma_1.prisma.user.update({
        where: { id: req.user.id },
        data,
        select: {
...
router.get("/notifications", async (req, res) => {
    ...
    const ownership = req.user?.role === "admin" || req.user?.role === "director"
        ? {}
        : { assignedToId: req.user?.id };
    const recentLogs = await prisma_1.prisma.activityLog.findMany({
        where: {
    ...
    const overdueTasks = await prisma_1.prisma.task.findMany({
        where: { ...ownership, status: "pending", dueDate: { lt: now } },
        select: { id: true, title: true, dueDate: true },
        take: 5,
    });
    const todayStart = new Date(now.getFullYear(), now.getMonth(), now.getDate());
    const newLeadsCount = await prisma_1.prisma.lead.count({
        where: { ...ownership, createdAt: { gte: todayStart } },
    });
    const notifications = [
        ...overdueTasks.map((t) => ({
            id: `task-${t.id}`,
...
router.post("/calendar/token", async (req, res) => {
    const token = (await Promise.resolve().then(() => __importStar(require("crypto")))).randomBytes(32).toString("hex");
...
router.get("/calendar/ics", async (req, res) => {
    ...
    for (const ev of events) {
        ...
        lines.push("BEGIN:VEVENT", `UID:${ev.id}@freemor`, `DTSTART:${d2(start)}`, `DTEND:${d2(end)}`, `SUMMARY:${esc(ev.title)}`);
        if (ev.description)
            lines.push(`DESCRIPTION:${esc(ev.description)}`);
        lines.push(`CATEGORIES:${ev.type || "other"}`, `DTSTAMP:${d2(new Date(ev.createdAt))}`, "END:VEVENT");
    }
...
router.get("/helper", async (req, res) => {
    ...
    const used = await prisma_1.prisma.helperMessage.count({
        where: {
            userId: req.user.id,
            role: "user",
    ...
    const history = await prisma_1.prisma.helperMessage.findMany({
...
router.post("/helper", async (req, res) => {
    ...
    await prisma_1.prisma.helperMessage.create({
...
router.post("/assistant", async (req, res) => {
    ...
    const reply = "Асистент у безпечному режимі після міграції. Інтелектуальний запит підключимо наступним кроком.";
    await prisma_1.prisma.helperMessage.create({
        data: {
            userId: req.user.id,
            role: "assistant",
...
router.put("/chat/rooms", async (req, res) => {
    ...
    if (name !== undefined)
        await prisma_1.prisma.chatRoom.update({ where: { id: roomId }, data: { name } });
    if (Array.isArray(addMemberIds)) {
        for (const mid of addMemberIds)
            await prisma_1.prisma.chatRoomMember.upsert({
                where: { roomId_userId: { roomId, userId: mid } },
                create: { roomId, userId: mid },
                update: {},
            });
        const count = await prisma_1.prisma.chatRoomMember.count({ where: { roomId } });
        if (count > 2)
    ...
    if (Array.isArray(removeMemberIds))
        await prisma_1.prisma.chatRoomMember.deleteMany({
            where: { roomId, userId: { in: removeMemberIds } },
        });
    res.json(await prisma_1.prisma.chatRoom.findUnique({
        where: { id: roomId },
        include: {
            members: {
                include: {
...
router.delete("/chat/rooms", async (req, res) => {
    ...
    if (!membership)
        return res.status(403).json({ error: "Not a member" });
    await prisma_1.prisma.chatMention.deleteMany({
