...
export function CalendarClient() {
  ...
  const fetchEvents = useCallback(async () => {
    ...
    const promises = [-1, 0, 1].map(offset => {
      ...
      return fetch(`/api/events?month=${mm}&year=${y}`).then(r => r.json()).catch(() => []);
    });
  ...
  const handleAddEvent = async (data: any) => {
    if (selectedEvent) {
      await fetch(`/api/events/${selectedEvent.id}`, { method: 'PUT', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(data) });
    } else {
    ...
    <div className="space-y-4" onClick={() => setPopoverEvent(null)}>
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
        ...
        <div className="flex items-center gap-2">
          ...
          <button onClick={() => openNewEvent(new Date().toISOString().slice(0, 10))}
...
function WeekView({ currentDate, today, daysShort, getEventsForDay, onSlotClick, onEventClick }: any) {
  ...
  useEffect(() => {
    if (scrollRef.current) {
      const now = new Date();
      const scrollTo = Math.max(0, (now.getHours() - 7.5) * 60);
      ...
      <div ref={scrollRef} className="overflow-y-auto max-h-[600px]">
        <div className="grid grid-cols-[56px_repeat(7,1fr)] relative">
          ...
          {days.some(d => isSameDay(d, today)) && (() => {
            const now = new Date();
            const top = (now.getHours() - 7 + now.getMinutes() / 60) * 60;
                ...
                return (
                  <div key={di}
                    ...
                    {dayEvents.map((ev: any) => {
                      const colors = EVENT_COLORS[ev.type] ?? EVENT_COLORS.other;
                      const startMin = new Date(ev.startDate).getMinutes();
                      return (
                        <div key={ev.id}
                          onClick={(e) => onEventClick(ev, e)}
                          ...
                          style={{ top: `${startMin}px`, minHeight: '24px' }}>
                  ...
                  {hourEvents.map((ev: any) => {
                    ...
                    return (
                      <div key={ev.id}
                        ...
                        style={{ top: `${startMin}px`, minHeight: '32px' }}>
                        <div className="font-semibold">{ev.title}</div>
                        <div className="text-[10px] opacity-75">{formatTime(new Date(ev.startDate))}</div>
                      </div>
                    );
                  })}
                </div>
...
function EventDialog({ date, event, onSave, onClose, t }: { date: string | null; event?: any; onSave: (d: any) => void; onClose: () => void; t: (k: string) => string }) {
  const initDate = event ? new Date(event.startDate).toISOString().slice(0, 16) : `${date ?? new Date().toISOString().slice(0, 10)}T10:00`;
