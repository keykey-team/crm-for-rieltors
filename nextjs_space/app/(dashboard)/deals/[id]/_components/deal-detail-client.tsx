...
import { formatPrice, formatDate, formatDateTime, getInitials } from '@/lib/format';
...
export function DealDetailClient({ dealId }: { dealId: string }) {
  ...
  const refreshPickerData = useCallback(() => {
    fetch('/api/leads').then(r => r.json()).then(d => setAllLeads(Array.isArray(d) ? d : [])).catch(() => {});
    fetch('/api/properties').then(r => r.json()).then(d => setAllProperties(Array.isArray(d) ? d : [])).catch(() => {});
  }, []);
  ...
  useEffect(() => {
    fetch('/api/users').then(r => r.json()).then(d => setTeamUsers(Array.isArray(d) ? d : [])).catch(() => {});
  }, []);
                ...
                <button
                  onClick={async () => {
                    await fetch(`/api/deals/${dealId}`, {
                      method: 'PUT',
                      headers: { 'Content-Type': 'application/json' },
                      body: JSON.stringify({ amount: financeForm.amount, commission: financeForm.commission, currency: financeForm.currency }),
                    });
                    toast.success(t('deals.saved'));
