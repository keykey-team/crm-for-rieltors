  ...
  if (loading) {
    return (
      <div className="flex gap-4 overflow-x-auto pb-4">
        {[1,2,3,4].map((i) => <div key={i} className="min-w-[260px] h-64 bg-card rounded-2xl animate-pulse" />)}
      </div>
    );
  }
  ...
  return (
    <div className="flex gap-3 sm:gap-4 overflow-x-auto pb-4 snap-x-mandatory" style={{ minHeight: '60vh' }}>
      {stages.map((stage: any) => {
        ...
        return (
          <div key={stage.value}
            className={`min-w-[75vw] sm:min-w-[220px] flex-1 rounded-2xl p-3 transition-all border ${
              dragOver === stage.value ? 'ring-2 ring-primary/30 bg-primary/5 border-primary/20' : 'bg-muted/30 dark:bg-muted/15 border-border/40 dark:border-border/30'
            }`}
            ...
            <div className="mb-3 px-1">
              ...
              {stageDeals.length > 0 && (() => {
                ...
                stageDeals.forEach((d: any) => {
                  const c = d?.currency || 'USD';
                  byCurrency[c] = (byCurrency[c] || 0) + (d?.amount ?? 0);
                });
                const entries = Object.entries(byCurrency).filter(([, v]) => v > 0);
                return entries.length > 0 ? (
                  <p className="text-[11px] font-mono font-semibold text-muted-foreground mt-1 pl-5">
            ...
            <div className="space-y-2">
              {stageDeals.map((deal: any) => (
                <div key={deal?.id} draggable
                  ...
                  {deal?.amount && (
                    <p className="text-xs font-mono font-bold text-primary mb-2">{formatPrice(deal.amount, deal.currency || 'USD')}</p>
                  )}
                  ...
                  {/* Progress bar */}
                  {(() => {
                    const idx = stages.findIndex((s: any) => s.value === stage.value);
