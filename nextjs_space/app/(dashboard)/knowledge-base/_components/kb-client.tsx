    ...
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <h1 className="text-2xl font-display font-bold tracking-tight flex items-center gap-2">
            <BookOpen className="w-6 h-6 text-primary" /> {t('kb.title')}
          </h1>
          <p className="text-sm text-muted-foreground mt-1">{t('kb.subtitle')}</p>
        </div>
      ...
      {loading ? (
        ...
        <div className="space-y-3">
          {(articles ?? []).map((a: any) => (
            <div key={a?.id} className="bg-white rounded-2xl overflow-hidden transition" style={{ boxShadow: 'var(--shadow-sm)' }}>
