...
import { Edit2, Trash2, Phone, MessageSquare, Workflow, ExternalLink, ArrowUpDown, ArrowUp, ArrowDown, Users } from 'lucide-react';
...
export function LeadTable({ leads, loading, onEdit, onDelete, onCall, onMessage, onStatusChange, onSourceChange, onManagerChange, managers = [], sortBy, sortDir, onSort, selectedIds, onToggleSelect, onToggleAll }: Props) {
  ...
  const [activeDropdown, setActiveDropdown] = useState<{ id: string; type: 'status' | 'source' | 'manager' } | null>(null);
  ...
  const toggleDropdown = (id: string, type: 'status' | 'source' | 'manager') => {
  ...
  const sortedLeads = useMemo(() => {
    ...
    arr.sort((a, b) => {
      ...
      switch (sortBy) {
        ...
        case 'manager': va = a.assignedTo?.name ?? ''; vb = b.assignedTo?.name ?? ''; break;
                  ...
                  <td className="px-4 py-3" onClick={e => e.stopPropagation()}>
                    <div className="relative inline-block" data-inline-dropdown>
                      <button onClick={() => toggleDropdown(lead.id, 'source')}
                        ...
                        {t(`const.leadSource.${lead.source}`) || source?.label || lead.source}
                      </button>
