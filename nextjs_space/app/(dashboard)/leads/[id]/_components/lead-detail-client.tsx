...
export function LeadDetailClient({ leadId }: { leadId: string }) {
  ...
  const fetchData = useCallback(async () => {
    try {
      ...
      const [leadData, commData] = await Promise.all([leadRes.json(), commRes.json()]);
      setLead(leadData);
      setComms(Array.isArray(commData) ? commData : []);
  ...
  const addComm = async () => {
    ...
    const res = await fetch('/api/communications', {
      ...
      body: JSON.stringify({ leadId, type: commType, direction: commDirection, content: newComm }),
    });
    if (res.ok) { setNewComm(''); fetchData(); toast.success(t('leads.addedComm')); }
  };

  const handleCreateDeal = async () => {
    setCreatingDeal(true);
    try {
      const res = await fetch(`/api/leads/${leadId}/create-deal`, { method: 'POST' });
      if (res.ok) {
        const deal = await res.json();
        toast.success(t('leads.dealCreated'));
        router.push(`/deals/${deal.id}`);
      } else { toast.error(t('leads.error')); }
