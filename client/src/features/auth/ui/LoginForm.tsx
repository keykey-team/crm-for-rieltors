...
import { loginWithCredentials } from '@/features/auth/lib/authService';
import { validateLoginPayload } from '@/features/auth/lib/validation';
...
export function LoginForm() {
  ...
  const handleSubmit = async (event: React.FormEvent) => {
    ...
    if (!result.ok) {
      setError(t(result.errorKey ?? 'auth.errorCredentials'));
      setLoading(false);
      return;
    }
          ...
          <div>
            ...
            <div className="relative">
              ...
              <input
                ...
                className="w-full pl-10 pr-10 py-2.5 rounded-xl border border-border bg-muted/30 focus:outline-none focus:ring-2 focus:ring-primary/20 focus:border-primary transition text-sm"
                placeholder="••••••••"
                required
              />
              <button
