...
import { signup } from '@/features/create-account/api/signup.api';
import { syncBackendSession } from '@/shared/lib/backend-auth';
import { parseForm, signupSchema } from '@/shared/lib/validation';
...
export function SignupForm() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    setError('');
    const validation = parseForm(signupSchema, { accountType, name, email, password });
    ...
    try {
      ...
      if (!data.ok) {
        // If account already exists, try to sign in with provided credentials.
        if (/already exists/i.test(data.error ?? '')) {
          const existsSignIn = await signIn('credentials', { email, password, redirect: false });
          if (!existsSignIn?.error) {
      ...
      const backendSynced = await syncBackendSession();
      if (!backendSynced) { setError(t('auth.errorServer')); setLoading(false); return; }
      router.replace('/dashboard');
          ...
          <div>
            ...
            <div className="relative">
              ...
              <input type="text" value={name} onChange={(e) => { setName(e.target.value); setErrors(p => ({ ...p, name: '' })); }}
                className={`w-full pl-10 pr-4 py-2.5 rounded-xl border bg-muted/30 focus:outline-none focus:ring-2 focus:ring-primary/20 focus:border-primary transition text-sm ${errors.name ? 'border-destructive/60' : 'border-border'}`}
                placeholder={accountType === 'agency' ? t('auth.agencyNamePlaceholder') : t('auth.namePlaceholder')} />
            </div>
          ...
          <div>
            ...
            <div className="relative">
              ...
              <input type="password" value={password} onChange={(e) => { setPassword(e.target.value); setErrors(p => ({ ...p, password: '' })); }}
                className={`w-full pl-10 pr-4 py-2.5 rounded-xl border bg-muted/30 focus:outline-none focus:ring-2 focus:ring-primary/20 focus:border-primary transition text-sm ${errors.password ? 'border-destructive/60' : 'border-border'}`}
