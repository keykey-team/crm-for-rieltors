...
import type { KnowledgeBaseArticle, KnowledgeBaseUpsertInput } from '@/entities/knowledge-base';
import { getKnowledgeBaseArticles, createKnowledgeBaseArticle, updateKnowledgeBaseArticle, deleteKnowledgeBaseArticle } from '@/entities/knowledge-base';
...
export function KBClient() {
  ...
  const fetchArticles = useCallback(async () => {
    setLoading(true);
    setArticles(await getKnowledgeBaseArticles({ search: search || undefined, category: category || undefined }));
    setLoading(false);
  }, [search, category]);

  useEffect(() => { fetchArticles(); }, [fetchArticles]);

  const handleSave = async (data: KnowledgeBaseUpsertInput) => {
    if (editArticle) await updateKnowledgeBaseArticle(editArticle.id, data);
    else await createKnowledgeBaseArticle(data);
    setDialogOpen(false);
    setEditArticle(null);
    fetchArticles();
  };
