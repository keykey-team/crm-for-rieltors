...
function ThreadPanel({
  ...
  const fetchReplies = useCallback(async () => {
    setReplies(await getThreadMessages(roomId, parentMsg.id));
  }, [roomId, parentMsg.id]);
  ...
  const sendReply = async (text: string, mentionIds: string[]) => {
    if (!text.trim()) return;
    await sendChatMessage({ roomId, text, threadId: parentMsg.id, mentions: mentionIds });
    setNewReply('');
    fetchReplies();
  };
...
function GroupSettingsPanel({
  ...
  const saveName = async () => {
    ...
    setSaving(true);
    try {
      const data = await updateChatRoom({ roomId: room.id, name: name.trim() });
  ...
  const addMember = async (uid: string) => {
    try {
      const data = await updateChatRoom({ roomId: room.id, addMemberIds: [uid] });
  ...
  const removeMember = async (uid: string) => {
    ...
    if (!ok) return;
    try {
      const data = await updateChatRoom({ roomId: room.id, removeMemberIds: [uid] });
      onUpdated({ ...room, members: data.members?.map((m: any) => m.user) || room.members });
    } catch { /* ignore */ }
  };
...
export function ChatClient() {
  ...
  const fetchMessages = useCallback(async () => {
    ...
    try {
      setMessages(await getChatMessages(selectedRoom.id));
    } catch { /* ignore */ }
  }, [selectedRoom]);
  ...
  const sendMessage = async (text: string, mentionIds: string[]) => {
    ...
    await sendChatMessage({ roomId: selectedRoom.id, text, mentions: mentionIds });
    setNewMsg('');
    fetchMessages();
    fetchRooms();
  };
  ...
  const startDM = async (user: MentionUser) => {
    ...
    // Create DM
    const room = await createChatRoom({ memberIds: [user.id] });
    fetchRooms();
